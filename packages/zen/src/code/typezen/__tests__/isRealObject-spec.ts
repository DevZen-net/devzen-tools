...
describe('isRealObject & its usage:', () => {
  describe('`z.isRealObject` distinquishes any kind of {} from any other types (->, [], Map, Set), even if {} is an instance (unlike lodashes functions that dont empower it).', () => {
    it('`z.isRealObject` recognises all {} as Objects', () => {
      expect(z.isRealObject({})).to.be.true
      expect(z.isRealObject(new (function Foo() {})())).to.be.true // unlike lodash _.isPlainObject
      expect(z.isRealObject(new (class FooBar {})())).to.be.true // unlike lodash _.isPlainObject
      expect(z.isRealObject(a_parentObj)).to.be.true // unlike lodash _.isPlainObject
      expect(z.isRealObject(a_childObj)).to.be.true // unlike lodash _.isPlainObject
      expect(z.isRealObject(a_Person)).to.be.true // unlike lodash _.isPlainObject
      expect(z.isRealObject(a_Employee)).to.be.true // unlike lodash _.isPlainObject
      expect(z.isRealObject(a_instance_withCommonProps)).to.be.true // unlike lodash _.isPlainObject
      expect(z.isRealObject(Object.create(null))).to.be.true
      expect(z.isRealObject(Object.create({}))).to.be.true // unlike lodash _.isPlainObject
    })

    it('`z.isRealObject` recognises all Arrays & Functions and Boxed items as NON Real Object', () => {
      expect(z.isRealObject([])).to.be.false
      expect(z.isRealObject(() => {})).to.be.false
      expect(z.isRealObject(Boolean(true))).to.be.false
      expect(z.isRealObject(new Boolean(true))).to.be.false
      expect(z.isRealObject(Number(12))).to.be.false
      expect(z.isRealObject(new Number(12))).to.be.false
      ...
      expect(z.isRealObject(new Map())).to.be.false
      expect(z.isRealObject(new Set())).to.be.false
      expect(z.isRealObject(new RegExp(/./))).to.be.false
      expect(z.isRealObject(new RegExp(/./))).to.be.false
      expect(z.isRealObject(/./)).to.be.false
    })
  ...
  describe('`_Β.isRealObject` returns false for all single values:', () => {
    for (const typeName in singleTypesAndPossibleValues) {
      for (const value of singleTypesAndPossibleValues[typeName]) {
        it(`\`z.isRealObject\` for '${typeName}':`, () => {
          expect(z.isRealObject(value)).to.be.false
        })
      }
    }
  })
