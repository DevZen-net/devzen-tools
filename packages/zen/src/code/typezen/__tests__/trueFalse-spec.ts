...
const falsies = [undefined, null, 0, NaN, '', BigInt(-0n), BigInt(0n), z.NOTHING]
const baddies = [...falseyBoxedPrimitives, z.NOTHING]
const allFalsifieds = [...falses, ...falseyBoxedPrimitives, ...falsies, ...baddies]

describe('TRUE & FALSE are not always straightforward in JS:', () => {
  describe('TRUE stuff:', () => {
    describe('z.isTrue is quite strict:', () => {
      describe('true with any strict boolean | Boolean true', () =>
        _.each(trues, (v) =>
          it(`value:'${toStringSafe(v)}'`, () => expect(z.isTrue(v)).to.be.true)
        ))
        ...
        _.each(
          ...
          (v) =>
            it(`false for value:'${toStringSafe(v)}'`, () =>
              !z.isEqual(v, new Boolean(true)) && expect(z.isTrue(v)).to.be.false)
        ))

      describe(`false with any falsified:`, () =>
        _.each(allFalsifieds, (v) =>
          it(`false for value:'${toStringSafe(v)}'`, () => expect(z.isTrue(v)).to.be.false)
        ))
    })

    describe(`z.isTruthy is a bit dumb`, () => {
      describe(`true with any truthy (good)`, () =>
        _.each(truthies, (v) =>
          it(`true for value:'${toStringSafe(v)}'`, () => expect(z.isTruthy(v)).to.be.true)
        ))

      describe(`false with any strictFalses & falsies (good)`, () =>
        _.each([...falses, ...falsies], (v) =>
          it(`false for value:'${toStringSafe(v)}'`, () => expect(z.isTruthy(v)).to.be.false)
        ))

      describe(`true with falseyBoxedPrimitives (bad) - except new Boolean(false)`, () =>
        _.each(falseyBoxedPrimitives, (v) =>
          it(`false for value:'${toStringSafe(v)}'`, () =>
            !z.isEqual(v, new Boolean(false)) && expect(z.isTruthy(v)).to.be.true)
        ))
    })
  ...
  describe('FALSE stuff:', () => {
    ...
    describe('z.isBad', () => {
      describe('false with all OKs', () =>
        _.each(OKs, (v) =>
          it(`*false* with value:'${toStringSafe(v)}'`, () => expect(z.isBad(v)).to.be.false)
        ))

      describe('false with all allFalsifieds', () =>
        _.each(allFalsifieds, (v) =>
          it(`*true* with value:'${toStringSafe(v)}'`, () => expect(z.isBad(v)).to.be.true)
        ))
    })
  })
})
