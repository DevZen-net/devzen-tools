  ...
  _.each(
    ...
    ([value, expected]) =>
      it(`for ${String(value)} returns ${expected}`, () => {
        expect(z.isRef(value)).to.be.equal(expected)
      })
  )

  describe('z.isRef correctly recognises "isMany" value types as NON-single:', () => {
    for (const typeName in manyTypesAndPossibleValues) {
      for (const value of manyTypesAndPossibleValues[typeName]) {
        it(`z.isRef recognises '${typeName}' value ${String(value)} as a "isRef" Type`, () => {
          expect(z.isRef(value)).to.be.true
        })
      }
    }
  })
