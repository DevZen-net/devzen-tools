...
import { delay } from '../../../test-utils/misc'
...
import { isNothing, NOTHING, STOP } from '../../typezen/utils'
...
// Copied from find-spec

describe('findKey.spec (copied from find.spec) - behavioural / implementation tests', () => {
  describe('z.findKey() - custom tests', () => {
    const array = [1, 2, 3, 10, 20, 30]
    const FIND_ITEM = 10
    const FIND_ITEM_POSITION = 4 // 1-based, not 0-based
    const FIND_ITEM_INDEX = 3 // 0-based

    describe('z.findKey() - item in simple array', () => {
      const input = array

      it(`find FIND_ITEM, stop iteration when found`, () => {
        let invocationCount = 0
        const reduceResult = findKey(input, (item) => {
          invocationCount++
          return item === FIND_ITEM
        })
        expect(reduceResult).to.equal(FIND_ITEM_INDEX)
      ...
      describe(`return STOP or STOP(value)`, () => {
        it(`find FIND_ITEM, with explicit STOP when found (return found item)`, () => {
          let invocationCount = 0
          const reduceResult = findKey(input, (item, idx) => {
            invocationCount++
            if (item === FIND_ITEM) return STOP(idx)
            return
          })
          expect(reduceResult).to.equal(FIND_ITEM_INDEX)
          expect(invocationCount).to.equal(FIND_ITEM_POSITION)
    ...
    describe('z.findKey() - item in AsyncGenerator', () => {
      ...
      it(`find FIND_ITEM, stop iteration when found`, async () => {
        let invocationCount = 0
        const reduceResult = await findKey(input, async (item) => {
          invocationCount++
          return item === FIND_ITEM
        })
        expect(reduceResult).to.equal(null) // AsyncIdxs are always null!
        expect(invocationCount).to.equal(FIND_ITEM_POSITION)
      })
      ...
      describe(`return STOP or STOP(value)`, () => {
        it(`find FIND_ITEM, with explicit STOP when found (return found count)`, async () => {
          let invocationCount = 0
          const reduceResult = await findKey(
            input,
            async (item, idxKeyIsNull, input, count) => {
              invocationCount++
              if (item === FIND_ITEM) return STOP(count)
              return
            }
          )
          expect(reduceResult).to.equal(FIND_ITEM_POSITION)
          expect(invocationCount).to.equal(FIND_ITEM_POSITION)
        })
        ...
        it(`find FIND_ITEM, with empty STOP when found (returns current item)`, async () => {
          ...
          expect(reduceResult).to.equal(null) // idxKey is always null for Generators
          expect(invocationCount).to.equal(FIND_ITEM_POSITION)
        })
      })
    })
  ...
  _.each(
    ...
    <[string, IKeysOptions, any, (string | symbol)[]][]>[
      ...
      [
        ...
        // [...arrayAndCommonStringProps_values, ...a_Array_of_Tvalues].map((item, idx)=> idx),
        // [...a_Array_of_Tvalues, ...arrayAndCommonStringProps_values].map((item, idx)=> idx),
      ] as any,
      [
      ...
      // // # ArrayBuffer DONT WORK WITH THESE TESTS, cause we read only PROPS / IDXs / Map Values on keys/idx
      //
      // [
    ...
    ([description, options, input, expected]: [
      ...
      describe(`z.find retrieves the 1st item, of all items of: "${description}" inserted into an array`, () => {
        let result: any
          ...
          try {
            result = findKey(
              ...
              (item, idxOrKey, input, count) =>
                count === 1 && idxOrKey !== null ? true : false,
        ...
        if (_.isError(expected))
          ...
          it(`equals 1st item`, () => {
            ...
            const foundIndex = expected.length === 0 ? NOTHING : 0
            // console.log({ input, expected, result, foundIndex })

            if (!isNothing(result)) {
              const valueInInput = options.props
                ? input[result]
                : isSet(input)
                  ? result
                  : isMap(input)
              ...
              expect(valueInInput).to.deep.equal(expected[foundIndex])
            }
          })
      })
    }
