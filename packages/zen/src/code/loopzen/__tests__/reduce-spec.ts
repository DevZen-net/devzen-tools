...
describe('reduce.spec (based on values.spec) - behavioural/implementation, no types check', () => {
  describe('z.reduce() - custom tests', () => {
    const array = [1, 2, 3, 10, 20, 30]

    describe(`z.reduce() - simple array`, () => {
      const input = array

      it(`reducing sum of array`, () => {
        let invocationCount = 0
        const reduceResult = reduce(input, (acc, item, idx) => {
          invocationCount++
          return acc + item
        }, 0)
        expect(reduceResult).to.equal(66)
        expect(invocationCount).to.equal(array.length)
      })

      it(`stops reducing, if STOP with returnValue is returned, returning returnValue as accumulator`, () => {
        let invocationCount = 0
        ...
        const reduceResult = reduce(
          ...
          (acc, item, idx) => {
            invocationCount++
            if (idx === STOP_INDEX) return STOP(acc + item)
        ...
        expect(reduceResult).to.equal(6) // NOT 66, because of STOP
        expect(invocationCount).to.equal(3) // 3 items, so 3 iterations
      })

      describe('z.reduce() - STOP reducing, include up to 3rd item', () => {
        it(`stops reducing, if plain empty STOP is returned, returning accumulator so far`, () => {
          let invocationCount = 0
          const reduceResult = reduce(
            input,
            (acc, item, idx) => {
              invocationCount++
              if (idx <= STOP_INDEX) return acc + item
              return STOP // return previous accumulator
            },
            0
          )

          expect(reduceResult).to.equal(6) // NOT 66, because of STOP
          expect(invocationCount).to.equal(4) // NOT 3, cause STOP is returned empty on next cycle, and previous accumulator is returned
        })
      })
    })
    describe(`z.reduce() - AsyncGenerator`, () => {
      ...
      it(`reducing sum of emmitted numbers`, async () => {
        const reduceResult = await reduce(input, (acc, item, idx) => acc + item, 0)
        expect(reduceResult).to.equal(66)
      })

      describe('z.reduce() - STOP reducing, include up to 3rd item', () => {
        it(`stops reducing, if STOP with returnValue is returned, returning returnValue as accumulator`, async  () => {
          let invocationCount = 1

          const reduceResult = await reduce(
            input,
            (acc, item, idxIsNull) => {
              if (invocationCount === STOP_INDEX + 1) return STOP(acc + item)
              invocationCount++
              return acc + item
            },
            0
          ...
          expect(reduceResult).to.equal(6) // NOT 66, because of STOP
          expect(invocationCount).to.equal(STOP_INDEX + 1)
        })

        it(`stops reducing, if plain empty STOP is returned, returning accumulator so far`, async () => {
          let invocationCount = 0

          const reduceResult = await reduce(
            input,
            async (acc, item, idxIsNull) => {
              invocationCount++
              if (invocationCount <= STOP_INDEX + 1) return acc + item
              return STOP // return previous accumulator
            },
            0
          )
          ...
          expect(reduceResult).to.equal(6) // NOT 66, because of STOP
          expect(invocationCount).to.equal(STOP_INDEX + 2) // +2 iterations from STOP_INDEX, cause we stop at next iteration, return empty STOP
        })
      })
    })
  })

  // Reduce returns the array of items (internal items and/or keys/props)
  _.each(
    <[string, IKeysOptions, any, (string | symbol)[]][]>[
      ...
      [
        `Throws: A Function with props & props: true & symbol: true & inherited: true, nonEnumerables: true because of 'caller', 'callee', and 'arguments' properties`,
      ...
      [
        ...
        undefined,
        new Error(
          `z.loop(): strict = true, but wrong z.isMany type. With type(value) === 'undefined' & value = undefined`
        ),
      ],
      [
        'null',
        ...
        null,
        new Error(
          `z.loop(): strict = true, but wrong z.isMany type. With type(value) === 'null' & value = null`
        ),
      ],
      [
        'number',
      ...
      [
        ...
        'a string',
        new Error(
          `z.loop(): strict = true, but wrong z.isMany type. With type(value) === 'string' & value = a string`
        ),
      ],
      [
        'boolean',
        ...
        true,
        new Error(
          `z.loop(): strict = true, but wrong z.isMany type. With type(value) === 'boolean' & value = true`
        ),
      ],
      [
        'bigint',
        ...
        1234n,
        new Error(
          `z.loop(): strict = true, but wrong z.isMany type. With type(value) === 'bigint' & value = 1234`
        ),
      ],
      [
        'symbol',
        ...
        Symbol('someLabel'),
        new Error(
          `z.loop(): strict = true, but wrong z.isMany type. With type(value) === 'symbol' & value = Symbol(someLabel)`
        ),
      ],
    ],
    ([description, options, input, expected]: [
      ...
      describe(`z.reduce retrieves all items of: "${description}" into an array`, () => {
        let result: any
