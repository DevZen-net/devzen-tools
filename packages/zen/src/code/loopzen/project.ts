...
const NO_RESULT = Symbol('z.NO_RESULT')
 ...
 *    - or `take` is a function and returns `false` or `z.STOP`.
 *
...
>(
  ...
  if (isUserClass(input))
    throw new Error(
      `z.${functionName}(): User classes are not supported - Cannot clone/map a class!`
    )
  ...
  if (functionName) {
    if (options[functionName])
      throw new Error(
        `z.${functionName}(): options.${functionName} can NOT be used here - use ${functionName}Cb only`
      )
  ...
  if (
    ...
    throw new Error(
      `z.project() as z.${functionName}(): Can't filter or take with filterSingles: false over an z.isSingleOrWeak value ${toStringSafe(input)}`
    )
  }
  ...
  if (_.isWeakMap(input) || _.isWeakSet(input))
    throw new Error(`z.${functionName}(): WeakMap & WeakSet are not supported!`)
  ...
  if (isPrimitive(input)) {
    if (singlesFilterTake(input as Titem, null, input))
      ...
      switch (type(input)) {
        ...
        default: {
          throw new Error(
            `z.${functionName}(): INTERNAL ERROR: Unsupported primitive type: ${type(input)}`
          )
        }
      }
    }
  ...
  if (
    ...
    if (options.strict && options.symbol && isAnyJustIterator(input))
      throw new TypeError(
        `z.${functionName}(): Cannot ${functionName} over an Iterator or an AsyncIterator with symbol: true & strict: true`
      )
  ...
  if (result === NO_RESULT)
    throw new Error(`z.${functionName}(): Unsupported value type: ${type(input)}`)
  else return result
}
