...
describe(`Single - Objects`, () => {
  describe(`# Promise`, () => {
    const valueTitle = `Promise` // just for title
    const a_Promise = Promise.resolve(a_Array_of_Tvalues)
    const a_Promise_withCommonProps = add_CommonProps(Promise.resolve(a_Array_of_Tvalues))

    const getValueVanilla = () => a_Promise // set to the "value" under test, eg `a_Array_of_Tvalues`, `aSetOfValues`, `a_Map_of_TMapKeys_Tvalues`, `aPojso`, `instance` etc
    const getValueWithExtraCommonProps = () => a_Promise_withCommonProps // like above, WECP
    ...
    type ExpectedInspectableNestedKeys = typeof isInspectableNested extends false ? never : number // if isInspectableNested is true, set the actual NestedKeys type, eg `number` for Array indexes, or Map/Set actual keys (without the array bit). If false, set to never.
    ...
    const expectedIteration_keys /*@todo(444): fix to `: TexpectedIteration_keys[]` */ = [null]
    ...
    // Iteration values
    type TexpectedIteration_values = ReturnType<typeof getValueVanilla>
    const expectedIteration_values: TexpectedIteration_values[] = [getValueVanilla()]
    ...
    type Texpected_stringNonEnumerablesOwn_values = Tpromise_prototype_stringNonEnumerablesOwn_values<typeof a_Promise>
    const expected_stringNonEnumerablesOwn_values = promise_prototype_stringNonEnumerablesOwn.map((value) => a_Promise[value])
    type Texpected_symbolNonEnumerablesOwn_values = never
    const expected_symbolNonEnumerablesOwn_values = []

    // inherited values
    type Texpected_stringNonEnumerablesInherited_values = Tpromise_prototype_stringNonEnumerablesInherited_values<typeof a_Promise>
    const expected_stringNonEnumerablesInherited_values = [...promise_prototype_stringNonEnumerablesInherited].map((value) => a_Promise[value])
    type Texpected_symbolNonEnumerablesInherited_values = Tpromise_prototype_symbolNonEnumerablesInherited_values<typeof a_Promise>
    const expected_symbolNonEnumerablesInherited_values = promise_prototype_symbolNonEnumerablesInherited.map((value) => a_Promise[value])

    // top values
    const expected_stringNonEnumerablesInheritedTop_values = object_stringNonEnumerablesInheritedTop.map((v) => a_Promise[v])
      ...
      describe('default/weird options - props assumed false. Returns Inspectable NestedKeys OR own enumerable props', () => {
        ...
        describe(`EmptyOptions`, () => {
          ...
          describe(`Expecting: Nested Keys & Values #R8`, () => {
            ...
            describe(`loop2()`, () => {
              it(`loop2() normal`, () => {
                const result = loop2(getValue(), options)
                expectType<TypeEqual<typeof result, Generator<[TexpectedIteration_values, TexpectedIteration_keys, CountType]>>>(true)
