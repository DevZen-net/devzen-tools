...
// DEPRECATED
export const eachWithOnly = (
  arr: object[],
  iteratee: (item, onlyMode: boolean) => void
) => {
...
export const delaySecs = async (secs = 1) => {
  ...
  if (typeof jest !== 'undefined')
    await jest.advanceTimersByTimeAsync(secs * 1000)
...
export const expectToThrow = (
  ...
  try {
    ...
    // console.log('expectToThrow() error = ', error)
    expect(errorsTranformation(error.message)).toMatch(
      errorsTranformation(expectedErrors)
    )
...
export const fnBody = (fn) => {
  ...
  return _.startsWith(_.trim(body), 'return')
    ? `${_.trim(body).slice(7)}`
    : body
}
