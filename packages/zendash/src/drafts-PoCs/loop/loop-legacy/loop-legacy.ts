 ...
 *
 * # _z.loop() Introduction
 *
 * Think of it as a "universal" `for...of loop(val) {...}`, aiming to be the only iterator style you'll ever need to use!
 *
 * Similarly to what `_.each()` tried to do more than a decade ago, using the same syntax/code for both "collections" of `{}` and `[]` values, now `_z.loop()` promises to work the same way, for any value type that has "many" values (and more).
 *
 * And of course it powers its own [`_z.each`](/functions/each.html) function, along with the usual [`map`](/functions/map.html), [`filter`](/functions/filter.html), [`take`](/functions/take.html), [`reduce`](/functions/reduce.html) etc offsprings.
 *
 * # _z.loop() Reasoning
 *
 ...
 *
 * With `_z.loop()` you can iterate on **any value** via:
 *
 *     for (const [value, idxOrKey] of _z.loop(anyValue)) {...}
 *
 * and it will work as expected everywhere.
 *
 * ZenDash `loop()` exposes all nested items/values as a tuple with 3 parts: `[item, idxOrKey?, count?]` for each item/element/entry of an [`_z.isMany()`](/functions/isMany.html) (or for [`_z.isSingle()`](/functions/isSingle.html) types as well if `loopSingles: true`).
 *
 ...
 *
 * ```javascript
 * for (const [val, key, count] of _z.loop(value, {
 ...
 *
 * for (const [item, idx, count] of _z.loop(array, {filter: v => v !== 'BAD'})) {
 ...
 *
 * This is powered by [`_z.keys`](/functions/keys.html) & fully supports [`IkeysOptions`](/interfaces/IkeysOptions.html), since [`IloopOptions`](/interfaces/IloopOptions.html) extends it!
 *
 ...
 * # Map & Set `.entries()`
 *
 * You can also pass `map.entries()` & `set.entries()` directly to `_z.loop()` and get the same loop behaviour.
 *
 ...
 *
 * All other [`_z.isSingleOrWeak`](/functions/isSingleOrWeak.html) values iterate on the single value itself as, **the first and only iteration**.
 *
 ...
 *
 * (Async) Iterators/Generators work out of the box, by returning an `Generator` or `AsyncGenerator`. You need to use the `for await` syntax to iterate on the `_z.loop()` result if input is an `AsyncIterator/Generator`:
 *
 * ```javascript
 ...
 * # Promises
 *
 * Currently, promises are treated as a single value, and not awaited. This means that if you pass a Promise to `_z.loop()`, it will iterate on the Promise itself (if `loopSingles: true`, and not await on the resolved value:
 *
 * ```javascript *
 * for (const [val, idx, count] of _z.loop(Promise.resolve([1, 2, 3, 4]))) {
 ...
 *
 * - It works the same for all these different _z.types such as `realObject`, `Array`, `Map`, `Set`, `Iterator`, `Generator`, both sync and Async!
 *
 ...
 *
 * - It extends [`_z.keys`](/functions/keys.html) to solve many limitations of iteration in Objects:
 ...
 *
 *   - Using `_z.loop(array)` respects sparse Arrays (unlike `for (let idx = 0; idx < arr.length; idx++) {...}` that doesn't, looping over non-existent items and producing `undefined` for them).
 *
 * - You can `filter`, `map`, `mapKeys` and `take` the items that are iterated over using the relevant callbacks in the options.
 *
 * - Type safe & Type strong: `_z.loop` returns the correct `item` & `idxOrKey` type for different input value types: arrays get an `[Titem, numericIdx]`, objects get `[Titem, string | symbol]`, Maps get `[Titem, any]` and so on, respecting the respective input types of nested items & keys passed ;-)
 *
 * @param anyValue any value (usually a collection / iterable / iterator / Map / Set). Note: `_z.loop()` has too many overloads, not listed here for brevity. Any value is accepted, and it will be iterated appropriately.
 *
