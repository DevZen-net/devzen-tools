...
describe(`Many, with Inspectable Nested Values`, () => {
  describe(`# Map: with Nested Values (Map<TMapKeys, Tvalues>) Note: values of Map are Tvalues`, () => {
    const valueTitle = `Map (with TMapKeys natural keys)` // just for title
    const getValueVanilla = () => a_Map_of_TMapKeys_Tvalues // set to the "value" under test, eg `a_Array_of_Tvalues`, `aSetOfValues`, `a_Map_of_TMapKeys_Tvalues`, `aPojso`, `instance` etc
    const getValueWithExtraCommonProps = () => a_Map_of_TMapKeys_Tvalues_WithCommonProps // like above, WECP
    ...
    type ExpectedInspectableNestedKeys = typeof isInspectableNested extends false ? never : TMapKeys // if isInspectableNested is true, set the actual NestedKeys type, eg `number` for Array indexes, or Map/Set actual keys (without the array bit). If false, set to never.
    const expectedInspectableNested_keys: ExpectedInspectableNestedKeys[] = !isInspectableNested ? [] : a_Array_ofKeys // An array of the actual natural keys of the value, eg for indexes [0,1,2,3], or Map/Set actual keys used. Leave as empty array, if `!isInspectableNested`

    // Iteration keys
    type TexpectedIteration_keys = TMapKeys
    const expectedIteration_keys = a_Array_ofKeys
    ...
    const expected_stringNonEnumerablesInherited_values = [...map_prototype_stringNonEnumerablesInherited].map((value) => a_Map_of_TMapKeys_Tvalues[value])
    ...
    const expected_symbolNonEnumerablesInherited_values = map_prototype_symbolNonEnumerablesInherited.map((value) => a_Map_of_TMapKeys_Tvalues[value])

    // top values

    const expected_stringNonEnumerablesInheritedTop_values = object_stringNonEnumerablesInheritedTop.map((v) => a_Map_of_TMapKeys_Tvalues[v])
