...
describe(`Many, with Inspectable Nested Values`, () => {
  describe(`# Array - with Nested Values (array elements)`, () => {
    const valueTitle = `Array` // just for title
    const getValueVanilla = () => a_Array_of_Tvalues // set to the "value" under test, eg `a_Array_of_Tvalues`, `aSetOfValues`, `a_Map_of_TMapKeys_Tvalues`, `aPojso`, `instance` etc
    const getValueWithExtraCommonProps = () => a_Array_of_Tvalues_withCommonAndArrayExtraProps // like above, WECP
    ...
    const expectedInspectableNested_keys: ExpectedInspectableNestedKeys[] = !isInspectableNested ? [] : a_Array_of_Tvalues_indexes // An array of the actual natural keys of the value, eg for indexes [0,1,2,3], or Map/Set actual keys used. Leave as empty array, if `!isInspectableNested`

    // Iteration keys
    type TexpectedIteration_keys = number
    const expectedIteration_keys = a_Array_of_Tvalues_indexes
    ...
    const expected_symbolOwnProps_values: Texpected_symbolOwnProps_values[] = commonSymbolProps_values // set to `commonSymbolProp_values`, or something else
    ...
    const expected_stringNonEnumerablesInherited_values = [
      // prettier-ignore
      ...array_prototype_stringNonEnumerablesInherited,
      ...(([] as any).findLastIndex ? array_prototype_stringNonEnumerablesInherited_ES2023 : []),
    ].map((value) => a_Array_of_Tvalues[value])
    ...
    const expected_symbolNonEnumerablesInherited_values = array_prototype_symbolNonEnumerablesInherited.map((value) => a_Array_of_Tvalues[value])

    // top values
    const expected_stringNonEnumerablesInheritedTop_values = object_stringNonEnumerablesInheritedTop.map((v) => a_Array_of_Tvalues[v])
    type Texpected_stringNonEnumerablesInheritedTop_values = ValueOf<Object, Exclude<keyof Object, keyof Array<any>>>
