...
describe(`Many, with Inspectable Nested Values`, () => {
  describe(`# Set: with Inspectable Nested Values (Set<Tvalues>) Note: keys of Set are just a_Array_of_Tvalues(), so _z.values() bring the values of the Set`, () => {
    const valueTitle = `Set (with Tvalues as natural keys/values)` // just for title
    const getValueVanilla = () => a_Set_of_Tvalues // set to the "value" under test, eg `a_Array_of_Tvalues`, `aSetOfValues`, `a_Map_of_TMapKeys_Tvalues`, `aPojso`, `instance` etc
    const getValueWithExtraCommonProps = () => a_Set_of_Tvalues_withCommonProps // like above, WECP
    ...
    type ExpectedInspectableNestedKeys = typeof isInspectableNested extends false ? never : Tvalues // if isInspectableNested is true, set the actual Nested Values type, eg Tvalues. If false, set to never.
    const expectedInspectableNested_keys: ExpectedInspectableNestedKeys[] = !isInspectableNested ? [] : a_Array_of_Tvalues // An array of the actual natural keys of the value, eg for indexes [0,1,2,3], or Map/Set actual keys used. Leave as empty array, if `!isInspectableNested`

    // Iteration keys
    type TexpectedIteration_keys = Tvalues
    const expectedIteration_keys = a_Array_of_Tvalues
    ...
    const expectedInspectableNested_values: ExpectedInspectableNestedKeys_values[] = !isInspectableNested ? [] : a_Array_of_Tvalues // An array of the actual Nested Values of the value, eg array elements or Map values / Set keys  used. Leave as empty array, if `!isInspectableNested`
    ...
    const expected_stringNonEnumerablesInherited_values = [...set_prototype_stringNonEnumerablesInherited].map((value) => a_Set_of_Tvalues[value])
    ...
    const expected_symbolNonEnumerablesInherited_values = set_prototype_symbolNonEnumerablesInherited.map((value) => a_Set_of_Tvalues[value])

    // top values
    const expected_stringNonEnumerablesInheritedTop_values = object_stringNonEnumerablesInheritedTop.map((v) => a_Set_of_Tvalues[v])
    type Texpected_stringNonEnumerablesInheritedTop_values = ValueOfStrict<Object, Exclude<keyof Object, keyof Set<any>>>

    describe(`loop2(): ${valueTitle} Vanilla, Nested Values only, without extra props #AR14`, () => {
      ...
      describe('default/weird options - props assumed false. Returns Inspectable NestedKeys OR own enumerable props', () => {
        describe(`NoOptions (special case, not using "Expecting: Nested Keys & Values", but identical apart from call`, () => {
          ...
          describe(`loop2()`, () => {
            ...
            it(`loop2() map`, () => {
              ...
              expectType<TypeEqual<typeof result, Generator<[T_NEW_TYPE, TexpectedIteration_keys, CountType]>>>(true)

              type ExpectedIterationKeys_mapped_SetKeysFollowValue = ReturnType<typeof getValue> extends Set<any> ? T_NEW_TYPE : TexpectedIteration_keys
              // @ts-expect-error @todo(212): Should Set item & key always follow each other, after a map/mapKeys operation?
              expectType<TypeEqual<typeof result, Generator<[T_NEW_TYPE, ExpectedIterationKeys_mapped_SetKeysFollowValue, CountType]>>>(true)
            })
          })
          it(`each()`, () => {})
          it(`map()`, () => {})
        ...
        describe(`EmptyOptions`, () => {
          ...
          describe(`Expecting: Nested Keys & Values #R9`, () => {
            it(`keys()`, () => {
    ...
    describe(`loop2(): ${valueTitle} WECP (With Extra Common Props) #AR4`, () => {
