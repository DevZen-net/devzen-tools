...
/* using _z.isXXX to construct some helpers */
// helper _z.isEqual & _z.isLike that prints the path where discrepancy was found
export const are = function (name, asEqual = true) {
  return function (a, b) {
    let path
    const isEq = _z[name](a, b, {
      path: (path = []),
      allProps: true,
      exclude: ['inspect'],
    })
    if (asEqual) {
      if (!isEq) {
        console.warn(
          `Discrepancy, expected \`true\` from _z.${name} \n at path:`,
          path.join('.'),
          '\n left value =',
          _z.getProp(a, path),
          '\n right value =',
      ...
      if (isEq) {
        console.warn(`Discrepancy, expected \`false\` from _z.${name}, but its \`true\`.`)
      }
...
export const createEqualSet = function (
  ...
  return function (result, expected) {
    ...
    const isEq = _z.isSetEqual(result, expected, comparator)
    if (asEqual) {
      if (!isEq) {
        console.warn(
          '\n createEqualSet _z.isSetEqual expected `true`',
          `\nexpected =`,
          expected,
          `\nresult =`,
          result,
      ...
      if (isEq) {
        console.warn('\n _z.isSetEqual expected `false`, got `true`', `\nresult =`, result)
      }
      expect(isEq).to.be.false
      return true
    }
