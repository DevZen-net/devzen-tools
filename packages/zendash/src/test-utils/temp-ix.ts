...
// // The problem with Ix
//
// // Issue #1: from() works only with Iterables, and not all values or objects in JS are Iterables. Most notable, the real plain old JS objects (the famous `{foo: 'bar'}`) object, is not Iterables. Singles are also not iterable naturally, but _z.loop() doesnt choke on them (unless you want to).
//
...
// }
//
// // The magic of `_z.loop()` is that it works with any value and not just Iterables. And it works the same, for all. And it conveys every possibly useful information, so it can unify the iteration in every aspect: high level or low level programming.
//
...
// }
//
// // Despite all those criticisms, IxJS has some great ideas within it (stemming from the amazing RxJS) and the `.pipe()` mechanism for one, is certainly very useful and might be adopted in future generations of ZenDash ( where `.pipe()` will simply be returns a new `LoopGenerator`, with iteration records projected & filtered according to the operators passed in `pipe()`).
//
// // On the other hand, _z.loop() has a different focus. It's not trying to be, at least in this version, a full-blown reactive or Pull or Push or Async or X specific use case programming library. It's only trying to get the *loop* right!
//
