...
/**
 * Customizer callback type for [`_z.isEqual`](/functions/isEqual.html).
 *
 * Checks for equality of two values, returning `true` or a "truthy" value if they are considered equal.
 *
 * If the customizer returns `undefined`, comparisons are handled by `_z.isEqual`.
 *
...
/**
 * Options interface for [`_z.isEqual`](/functions/isEqual.html)
 */
export interface IsEqualOptions extends IKeysOptions {
   ...
   *
   * If `undefined` or if the function call returns `undefined`, comparisons are handled by `_z.isEqual`.
   *
...
/**
 * Default options for [`_z.isEqual`](/functions/isEqual.html) *
 */
...
export function isEqual(
  ...
  b: any,
  /**
   * The function to customize comparisons. If `undefined`, comparisons are handled by `_z.isEqual`. If `customizer` is passed as a property of the `options` object, it will be used instead of this argument. You can also pass the `options` object as the 3rd argument, in place of `customizer`.
   */
  customizer?: IsEqualCustomizerCallback,
  /**
 ...
 *
 * Note: it is NOT 100% compatible to `lodash`, mostly for good reasons. Few minor edge cases are not implemented yet as `options`, and current default deemed better and so kept different to lodash. Search `_z.isEqual DIFFERENT` in `src/code/objects/isEqual-lodash-spec.ts`, where lodash tests are also tested against `_z.isEqual`, with differences highlighted.
 *
 * @param a - The first value to compare
 * @param b - The second value to compare
 * @param customizer - The function to customize comparisons. If `undefined`, comparisons are handled by `_z.isEqual`. If `customizer` is passed as a property of the `options` object, it will be used instead of this argument. You can also pass the `options` object as the 3rd argument, in place of `customizer`.
...
export function isEqual(
  ...
  if (isRealObject(_customizerOrOptions)) {
    // prettier-ignore
    if (_options) throw new Error('_z.isEqual: you cannot pass options as 3rd and 5th argument at the same time')
  ...
  if (_options)
    if (isRealObject(_options)) {
      ...
      throw new Error('_z.isEqual: options must be an object of type IisEqualOptions')
    }
  ...
  if (
    ...
    const err = new Error(
      '_z.isEqual: 3rd arg must be a function customizer or an object of type IisEqualOptions'
    )
  ...
  if (options.customizer && !_.isFunction(options.customizer))
    throw new Error('_z.isEqual: options.customizer must be a function')
  ...
  if (ctx && options.ctx)
    throw new Error(
      '_z.isEqual: you cannot pass ctx as 4th argument & as options.ctx at the same time'
    )
...
const isEqualInternal = (
  ...
  if (options.props !== true && (isSingle(a) || _.isArrayBuffer(a) || _.isError(a))) {
    // @todo: implement _z.isEqual for these types, dont rely on isEqual
