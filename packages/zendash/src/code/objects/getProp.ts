...
/**
 * Default options for [`_z.getProp`](/functions/getProp.html)
 */
...
/**
 * Options interface for [`_z.getProp`](/functions/getProp.html)
 */
 ...
 *
 * _z.getProp(o, '/a/b') // returns { c: 'value at a/b/c' }
 * _z.getProp(o, 'a.b.c', {separator: '.'}) // returns 'value at a/b/c'
 * _z.getProp(o, ['a', 'b', 'c'], {fullResult: true}) // returns ['value at a/b/c', ['a', 'b', 'c'], true]
 * ```
 *
 ...
 *
 * @returns {any | [any, string[]], boolean} the value found, or `['valueFound', ['path', 'that', 'was', 'walked'], true]` if `fullResult` is true, that can be used as `const [value, walkedPath, isFound] = _z.getProp(o, {fullResult: true})` and:
 *
    ...
    if (_.isString(path)) {
      ...
      if (_.isNumber(path)) {
        ...
        if (path === undefined) {
          ...
          throw new Error(`ZenDash: _z.getProp Error: invalid path: ${path}`)
        }
      }
    }
  }
