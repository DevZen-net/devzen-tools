// Tests run in both 'ZenDash _z.isEqual' & those supported in 'Lodash _.isEqual'
//  - All differences are marked with '### _z.isEqual DIFFERENT: xxx' - these are potentially BREAKING CHANGES, if you plan to use ZenDash _z.isEqual as a drop in replacement of Lodash _.isEqual
//  - All extra `_z.isEqual` functionalities (compared to _.isEqual) are marked with '### _z.isEqual EXTRA FUNCTIONALITY: xxx'
...
const lodashIsEqualSkipOptions = (val, other, ...rest) => {
  if (!_.isEmpty(rest)) {
    ...
    // just reuse _z.isEqual for this call!
    return _z.isEqual(val, other, ...rest)
  }
  return _.isEqual(val, other)
}

_.each([_z.isEqual, lodashIsEqualSkipOptions], (isEqual) => {
  const isZisEqual = isEqual === _z.isEqual
  const isEqualWith: any = isZisEqual ? isEqual : _.isEqualWith
  const trueIfZenDash = isZisEqual ? 'true' : 'false'
  const falseIfZenDash = isZisEqual ? 'false' : 'true'

  describe(`ZenDash'es isEqual tests, both against ZenDash & Lodash (where supported): ${
    isZisEqual ? 'ZenDash _z.isEqual' : 'Lodash _.isEqual'
  }`, () => {
    describe(`Argument validation - if args are not valid, it throw errors`, () => {
      it('options are used as 3rd & 5th arg', () =>
        ...
        expect(() => isEqual(1, 2, {}, undefined, {})).to.throw(
          /_z.isEqual: you cannot pass options as 3rd and 5th argument at the same time/
        ))

      it('customizer is used as 3rd ard & in options', () =>
        // @ts-ignore
        expect(() => isEqual(1, 2, () => {}, undefined, { customizer: () => {} })).to.throw(
          /_z.isEqual: you cannot pass customizer as 3rd argument and as options.customizer at the same time/
        ))
    })
    ...
    describe('customizer', () => {
      describe('customizer basics: ### _z.isEqual EXTRA FUNCTIONALITY: customizer doesnt need _.isEqualWith, you can use same _.isEqual with an extra arg', () => {
      ...
      if (isZisEqual) {
        describe(`customizer & ctx: ### _z.isEqual EXTRA FUNCTIONALITY: customizer be bound to ctx`, () => {
        ...
        describe('customizer in `options.customizer`: called the right number of times, with the right arguments: ### _z.isEqual EXTRA FUNCTIONALITY: customizer can be in options, with the same `isEqual()`, no need for `isEqualWith()`', () => {
          const ctx = {}
    ...
    describe('Equality checks of different types:', () => {
      ...
      describe('Boxed Primitives & Unboxing: ### _z.isEqual DIFFERENT: lodash _.isEqual does unboxing by default, _z.isEqual requires `unbox: true`', () => {
        describe('Boolean', () => {
          describe('Without unboxing', () => {
            it(`true / new Boolean(true)`, () => {
              expect(isEqual(new Boolean(true), true)).to.be[falseIfZenDash]
              expect(isEqual(true, new Boolean(true))).to.be[falseIfZenDash]
            })

            it(`false / new Boolean(false)`, () => {
              expect(isEqual(new Boolean(false), false)).to.be[falseIfZenDash]
              expect(isEqual(false, new Boolean(false))).to.be[falseIfZenDash]
            })
          })
          if (isZisEqual)
            describe('With unboxing - ZenDash _z.isEqual only', () => {
              it(`true / new Boolean(true)`, () => {
        ...
        describe('Number', () => {
          describe('Without unboxing', () => {
            it(`1 / new Number(1)`, () => {
              expect(isEqual(new Number(1), 1)).to.be[falseIfZenDash]
              expect(isEqual(1, new Number(1))).to.be[falseIfZenDash]
            })

            it(`0 / new Number(0)`, () => {
              expect(isEqual(new Number(0), 0)).to.be[falseIfZenDash]
              expect(isEqual(0, new Number(0))).to.be[falseIfZenDash]
            })
          })
          if (isZisEqual)
            describe('With unboxing - ZenDash _z.isEqual only', () => {
        ...
        describe('String', () => {
          describe('Without unboxing', () => {
            it(`"a" / new String("a")`, () => {
              expect(isEqual(new String('a'), 'a')).to.be[falseIfZenDash]
              expect(isEqual('a', new String('a'))).to.be[falseIfZenDash]
            })

            it(`"" / new String("")`, () => {
              expect(isEqual(new String(''), '')).to.be[falseIfZenDash]
              expect(isEqual('', new String(''))).to.be[falseIfZenDash]
            })
          })
          if (isZisEqual)
      ...
      describe('Objects', () => {
        ...
        describe(`pojsos VS instance of same data`, () => {
          ...
          it('are NOT equal - same as lodash: search "should compare object instances" in lodash tests', () => {
            ...
            expect(_z.isIqual(b, a)).to.be.false
          })

          if (isZisEqual)
            it('they are equal with `options.realObjectAsPojso: true - ### _z.isEqual EXTRA FUNCTIONALITY: options.realObjectAsPojso`', () => {
              ...
              expect(_z.isIqual(b, a, { realObjectAsPojso: true })).to.be.true
            })
        })
        ...
        describe('TallKeysOptions Options: objects with symbol keys', () => {
          ...
          describe(`clones differing on Symbols`, () => {
            it('notACloneCauseOfSymbol are equal, with symbol: false (default): ### _z.isEqual DIFFERENT false if symbol properties are different, only with `options.symbol: true`', () =>
              expect(isEqual(a, notACloneCauseOfSymbol)).to.be[trueIfZenDash])

            if (isZisEqual)
              it('notACloneCauseOfSymbol are NOT equal, if we have `symbol: true` - ZenDash _z.isEqual only', () =>
                expect(
        ...
        describe('Sets with options.exactKeys: match exact key refs OR clones', () => {
          ...
          describe('options.exactKeys: false (default)', () => {
            ...
            describe('isEqualWith', () => {
              it('Clone keys with _z.isEqual - Sets are equal', () => {
                expect(isEqualWith(setCloneKeys1, setCloneKeys2, (a, b) => _z.isEqual(a, b))).to
                  .be.true
              })
      ...
      describe('Map', () => {
        ...
        describe(`'Maps with options.exactKeys & options.exactValues: match exact key/value refs OR clones`, () => {
          ...
          describe(`options.exactKeys: false & exactValues: false (default)`, () => {
            ...
            describe('isEqualWith', () => {
              it('Clone keys with _z.isEqual - Maps are equal', () => {
                expect(isEqualWith(mapCloneKeys1, mapCloneKeys2, (a, b) => _z.isEqual(a, b))).to
                  .be.true
              })
              ...
              it('Maps with same deep equal items _z.isEqual - Maps are equal', () =>
                expect(
                  isEqualWith(
                    get_Map_of_TMapKeys_Tvalues(),
                    get_Map_of_TMapKeys_Tvalues(),
              ...
              it('Nested Maps & objects with _z.isEqual are equal', () => {
                expect(
                  isEqualWith(
      ...
      describe('functions, with/without props', () => {
        ...
        it('isExact', () => expect(_z.isExact(f1, f2)).to.be.false)
        it('isIqual', () => expect(_z.isIqual(f1, f2)).to.be.false)
    ...
    describe("Value's `isEqual` function: ### _z.isEqual EXTRA FUNCTIONALITY: uses `isEqual` method if it exists on either side, lodash doesnt at all!", () => {
      class Aclass {
      ...
      it('recognises & uses `isEqual` method on either value sides', () => {
        // uses `isEqual` method on ZenDash,
        expect(isEqual(aValue, bValueEqualToA)).to.be[trueIfZenDash]
        expect(isEqual(bValueEqualToA, aValue)).to.be[trueIfZenDash]
        expect(isEqual(aValue, { val: 'YEAH' })).to.be[trueIfZenDash]
        expect(isEqual({ val: 'YEAH' }, bValueEqualToA)).to.be[trueIfZenDash]
      ...
      if (isZisEqual)
        it('isEqual can be disabled with `options.isEqualMethod: false` - ZenDash _z.isEqual only', () => {
    ...
    describe('options.inherited - Objects with inherited properties:', () => {
      describe('object with inherited properties:', () => {
        ...
        describe.skip('with _.cloneDeep:', () => {
          it('_z.isIqual fails (imperfect _.cloneDeep)', () => {
            expect(_z.isIqual(oClone, expectedPropertyValues)).to.be.false
            expect(_z.isIqual(expectedPropertyValues, oClone)).to.be.false
          })
          it('_.isEqual fails', () => {
        ...
        // return describe.skip('with _z.clone proto: ', () =>  {
        //   var oCloneProto
        //   oCloneProto = _z.clone(objectWithProtoInheritedProps, {
        //     copyProto: true,
        //   })
        //   it('_z.isIqual succeeds (a perfect clone:-)', () =>  {
        //     expect(_z.isIqual(oCloneProto, expectedPropertyValues)).to.be.true
        //     expect(_z.isIqual(expectedPropertyValues, oCloneProto)).to.be
        //       .true
        //   })
      ...
      describe('coffeescript object with inherited properties:', () => {
        it.skip('_z.isEqual is true', () => {
          ...
          expect(_z.isIqual(expectedPropertyValues, c3)).to.be.true
        })
        it('_.isEqual fails', () => {
        ...
        describe('with _.cloneDeep:', () => {
          it.skip('_z.isIqual fails (imperfect _.cloneDeep)', () => {
            expect(
              isEqual(c3Clone, expectedPropertyValues, undefined, undefined, {
                inherited: true,
              })
            ).to.be.false
            expect(_z.isIqual(expectedPropertyValues, c3Clone)).to.be.false
          })
          it('_.isEqual fails', () => {
        ...
        // describe('with _z.clone: ', () =>  {
        //   var c3CloneProto
        //   c3CloneProto = _z.clone(c3, {
        //     copyProto: true,
        //   })
        //   it('_z.isIqual is true', () =>  {
        //     expect(
        //       _z.isEqual(c3CloneProto, expectedPropertyValues, {
        //         inherited: true,
        //         exclude: ['constructor'],
        //       })
        //     ).to.be.true
        //     expect(_z.isIqual([expectedPropertyValues], [c3CloneProto])).to
        //       .be.true
        //   })
        //   it('_.isEqual fails', () =>  {
    ...
    describe('options.exact (Objects need to have exact refs, i.e shallow clones) :', () => {
      describe('shallow cloned objects :', () => {
        it('_z.isExact(object, objectShallowClone1) is true', () => {
          expect(
            isEqual(object, objectShallowClone1, undefined, undefined, {
              exactValues: true,
            })
          ).to.be.true
          expect(_z.isExact(objectShallowClone1, object)).to.be.true
        })
        it('_z.isExact(object, objectShallowClone2) with _.cloneDeep(object) is true', () => {
          expect(
            isEqual(object, objectShallowClone2, {
              exactValues: true,
            })
          ).to.be.true // alt `options` passing style (in customizer's place)
          expect(_z.isExact(objectShallowClone2, object)).to.be.true
        })
      ...
      describe('deeply cloned objects:', () => {
        ...
        describe('objectDeepClone2 = _.cloneDeep(object)', () => {
          it('_z.isExact is false', () => {
            ...
            expect(_z.isExact(objectDeepClone2, object)).to.be.false
          })
          it('_z.isEqual is true', () => {
      ...
      describe('isIxact : isEqual with inherited & exact :', () => {
        describe('shallow inherited clone: inheritedShallowClone:', () => {
          it('isIxact is true:', () => {
            expect(_z.isIxact(inheritedShallowClone, object)).to.be.true
          })
          it('isIqual is true:', () => {
            expect(_z.isIqual(object, inheritedShallowClone)).to.be.true
          })
        })
        describe('deep inherited clone : inheritedDeepClone:', () => {
          it('isIxact is true:', () => {
            expect(_z.isIxact(inheritedDeepClone, object)).to.be.false
          })
          it('isIqual is true:', () => {
            expect(_z.isIqual(object, inheritedDeepClone)).to.be.true
          })
        })
      })
    })
    ...
    if (isZisEqual) {
      describe('_z.isLike / _z.isEqual with {like:true}: (1st arg can be a partial of 2nd arg)', () => {
        ...
        it("_z.isLike: is true if 1st args's properties match to 2nd arg", () =>
          expect(_z.isLike(obj1IsLikeObj2, obj2)).to.be.true)
        it("_z.isEqual with {like: true}: is true if 1st args's properties match to 2nd arg", () => {
          expect(_z.isEqual(obj1IsLikeObj2, obj2)).to.be.false
          expect(_z.isEqual(obj1IsLikeObj2, obj2, { like: true })).to.be.true
        })

        it("_z.isLike: is false if 1st args's properties are not like to 2nd arg", () => {
          expect(_z.isLike(obj2, obj1IsLikeObj2)).to.be.false
        })
        it("_z.isEqual with {like: true}: is false if 1st args's properties are not like to 2nd arg", () => {
          expect(_z.isEqual(obj2, obj1IsLikeObj2)).to.be.false
          expect(_z.isEqual(obj2, obj1IsLikeObj2, { like: true })).to.be.false
        })
      })
      ...
      describe(`'Maps with options.exactKeys & options.exactValues: match exact key/value refs OR clones`, () => {
        ...
        describe(`options.exactKeys: false & exactValues: false (default)`, () => {
          it('_z.isLike', () => {
            expect(_z.isEqual(mapCloneKeys1, mapCloneKeys2, { like: true })).to.be.true
          })

          it('_z.isEqual with {like: true}', () => {
            expect(_z.isEqual(mapCloneKeys1, mapCloneKeys2)).to.be.false
            expect(_z.isEqual(mapCloneKeys1, mapCloneKeys2, { like: true })).to.be.true
          })
        })
      })

      describe('aliases like _z.isLike & other options:', () => {
        const a1 = {
          a: 1,
          b: {
            b1: 1,
        ...
        it('no options, just isLike', () => expect(_z.isLike(b1, a1)).to.be.false)
        ...
        it('pass options in 5th place', () => {
          let path
          expect(
            _z.isLike(b1, a1, undefined, undefined, {
              path: (path = []),
            })
          ).to.be.false
          expect(path).to.be.deep.equal(['b', 'b2'])
        ...
        it('passes customizer & options in its proper place & as an option', () => {
          ...
          expect(
            _z.isLike(b1, a1, customizer, undefined, {
              path: (path = []),
            })
          ).to.be.true
        ...
        describe(`Argument validation - if args are not valid, it throw errors`, () => {
          it('options are used as 3rd & 5th arg', () =>
            // @ts-ignore
            expect(() => _z.isLike(a1, b1, {}, undefined, {})).to.throw(
              /_z.isEqual: you cannot pass options as 3rd and 5th argument at the same time/
            ))
          ...
          it('customizer is used as 3rd ard & in options', () =>
            // @ts-ignore
            expect(() =>
              _z.isLike(a1, b1, () => {}, undefined, { customizer: () => {} })
            ).to.throw(
              /_z.isEqual: you cannot pass customizer as 3rd argument and as options.customizer at the same time/
            ))
        })
