...
/**
 * Options interface for [`_z.setProp`](/functions/setProp.html)
 */
...
/**
 * Default options for [`_z.setProp`](/functions/setProp.html)
 */
 ...
 *
 *  - `create: boolean = false` if truthy, it creates a new `{}` for missing paths. If falsey, if there is a non [`_z.isRealObject`](/functions/isRealObject.html) while expanding the path, it doesn't create a new `{}` and it doesn't set the value. Default is `false`.
 *
 *  - `overwrite: string | boolean = false` if true (or truthy string), objects paths that don't exist are created, just like `create: true`. But also, non [`_z.isRealObject`](/functions/isRealObject.html) values are replaced by a new `{}` that will 'hold' this new path. If it is a String, then a key/prop by that name is created on the new `{}`, holding the old value that was overwritten. Default is `false`.
 *
...
export const setProp = (
  ...
  if (!_.isArray(paths)) {
    if (_.isString(paths)) {
      ...
      throw new TypeError(
        `_z.setProp Error: invalid path: ${paths}. Use either an Array, eg ['path1', 'path2'] or separator-ed String, eg) 'path1.path2'`
      )
  }

  if (!_.isObject(objectOrArray))
    throw new TypeError(`_z.setProp Error: invalid object: ${objectOrArray}`)
