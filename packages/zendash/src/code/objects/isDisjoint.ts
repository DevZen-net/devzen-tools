...
/**
 * Default options for [`_z.isDisjoint`](/functions/isDisjoint.html)
 */
...
/**
 * Options interface for [`_z.isDisjoint`](/functions/isDisjoint.html)
 */
...
export const isDisjoint = (
  ...
  if (set1Type !== 'Set' && set1Type !== 'Array' && set2Type !== 'Set' && set2Type !== 'Array')
    throw new Error(
      `_z.isSetEqual(): Both sets must be either of type (Array or Set), but got '${set1Type}' & '${set2Type}'`
    )
