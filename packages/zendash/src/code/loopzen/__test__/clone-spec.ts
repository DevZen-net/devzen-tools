...
describe('clone() - creates a new value (object, Array or anything else), with the input value contents verbatim', () => {
  ...
  describe(`clone throws on Iterators & Generators, cause they can't be restarted`, () => {
    const errorMsg = `_z.clone(): Cannot clone an Iterator/Generator or an Async Generator/Iterator, cause they cant be restarted! You can map over the **remaining items** of the iterator by using \`_z.map()\` instead`
    it(`Iterator`, () =>
