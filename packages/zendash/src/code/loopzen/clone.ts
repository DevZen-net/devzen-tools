 ...
 *
 * @see [`_z.map`](/functions/map.html) as `_z.clone()` is just shortcut to `_z.map(value, options, _.identity)`, plus some extra sanity checks. In turn `_z.map()` is powered by the mighty `_z.loop()`.
 *
 * @see [`_z.loop()`](/functions/loop.html) the power hidden behind `map()` and most other collection functions.
 *
 ...
 *
 * But Iterators & Generators etc dont make sense to be cloned per se, so these throw. Reason is that these can't be restarted, and it would signal the wrong ida. You can always `_z.map()` over their remaining items, with an `_z.identity` and that's effectivelly a `_z.clone()`, but semantics differ.
 */
...
>(
  ...
  try {
    ...
    throw new Error(
      `_z.clone(): Could not clone the given value: ${error instanceof Error ? error.message : error}`
    )
  }
}
