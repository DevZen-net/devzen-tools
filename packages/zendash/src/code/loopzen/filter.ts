 ...
 *
 * Similar idea to lodash `_.filter()` or `Array.filter()` but powered by [`_z.loop()`](/functions/loop.html). Hence:
 *
 ...
 * # Other/Single Values
 *
 * All of these currently are **optionally** supported by `_z.filter` (with `filterSingles: true`):
 *
 ...
 *
 * - Powered by the mighty [`_z.loop()`](/functions/loop.html) / [`_z.keys`](/functions/keys.html), so and hence you can control which keys / idx are visited (own / inherited / enumerable / string / symbol etc) via [`IloopOptions`](/interfaces/IloopOptions.html).
 *
 * - [`_z.isSingle`](/functions/isSingle.html) values are also supported: the projection callback will be called only once (with `options.strict: false` - default) or throw an error (with `options.strict: true`). This follows the functional programming principles, that all values are enclosed and can be mapped over. This is also the reason why `options.strict: false` is the default.
 *
