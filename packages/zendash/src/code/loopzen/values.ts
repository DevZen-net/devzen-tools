 ...
 *
 * The features, logic & options (of `KeysOptions`) are identical to [`_z.keys`](/functions/keys.html), but it returns the nested/props **values** instead of the nested **keys/props**. See [`Keys`](/functions/Keys.html) for more details.
...
export function values<Tinput, Toptions extends IKeysOptions = IKeys_DefaultOptions>(
  ...
  if (!_.isObject(input)) {
    ...
    if (isPrimitive(input)) return []
      ...
      const msg = `_z.values: internal error: value is not isPrimitive, but not an _.isObject either! You might need to check logs for value.`
  ...
  if (isArrayBuffer(input)) {
    ...
    if (!options.props || options.props === 'all') {
      // prettier-ignore
      if (!options.dataViewType) throw new Error(`_z.values(): options.dataViewType is required to loop over an ArrayBuffer to pick its nested values()`)
