 ...
 *
 * Similar idea to lodash `_.map()` or `Array.map()` or `_.mapValues()`, but powered by [`_z.loop()`](/functions/loop.html). Hence:
 *
 ...
 *
 * - [`_z.isSingle`](/functions/isSingle.html) values (primitives & boxed) are also supported: the projection callback will be called only once (by default, with `options.loopSingles: true` & `options.strict: false`) or throw an error (with `options.strict: true`). This follows the functional programming principles, that all values are enclosed and can be mapped over. This is also the reason why `options.strict: false` is the default.
 *
 ...
 *
 * - Powered by the mighty [`_z.loop()`](/functions/loop.html) / [`_z.keys`](/functions/keys.html) & [`_z.values`](/functions/values.html), hence you can control which keys / idx are visited (`own` / `inherited` / `nonEnumerables` / `string` / `symbol` etc) via [`IloopOptions`](/interfaces/IloopOptions.html).
 *
