...
> extends IKeysOptions,
    IFilterOnlyOptions {
  /**
   * If `true`, only `isMany` input values (according to `_z.isMany` or `option.allProps`) are allowed and iterated over (excluding `WeakMap` & `WeakSet` which can't be iterated at all). All other `_z.isSingle` values (& `WeakMap` & `WeakSet`) throw an error.
   *
   ...
   *    for (const [item, idx] of _z.loop(promises) console.log(await item, idx)
   *
   ...
   *
   * for await (const [item, idx, count] of _z.loop(promises, {asyncUnwrap: true, filter: oddNumbers})) {
  ...
  /**
   * Async option: NOTE: it applies only when looping with `_z.each()`, not the normal `_.zloop()`
   *
   ...
   *
   *   _z.each(persons, async (person, idxKey) => registerPersonPromise(person))
   *
   ...
   *
   *    _z.each(persons, (person, idxKey) => registerPersonPromise(person) )
   *
   *    // DOES NOT wait until each person is registered, before continuing to the next. Effectively, all promises are dispatched simultaneously.
   * ```
   *
   ...
   *
   * ```javascript
   *    _z.each(persons, (person, idxKey) => registerPersonPromise(person), {async: true})
   *
   *    // waits until each person is registered, before continuing to the next.
   * ```
   *
   ...
   *
   * ```javascript
   *    _z.each(persons, (person, idxKey) => registerPersonPromise(person), {async: 3})
   *
...
/**
 * The sole purpose of callbacks to exist here in `IloopCallbacks`, separately from [`ILoopOptions`](/interfaces/ILoopOptions.html), is due to a ZenDash/TypeScript limitation, that doesn't properly infer the type of a callback's arguments (it ignores the options, probably cause they belong to same type interface before it has been initialized).
 *
...
export interface IFilterOnlyOptions {
  ...
  /**
   * If `filterSingles: false` (default), it throws an error on any `_z.isSingleOrWeak` input (eg `_z.filter(123)` throws), as well as for `WeakMap` & `WeakSet` which are treated as singles (they can't be iterated over).
   *
  ...
  /**
   * If `filterSingles: false` (default), it throws an error on any single input (eg `_z.filter(123)` throws).
   *
   * BUT:
   *
   ...
   *
   * If `singlesReject` is **truthy**, it returns that value. **Note that the default** is the special value `_z.NOTHING` (which equals to `Symbol('_z.NOTHING')`)
   *
   * If `singlesReject` is **falsey**, it returns what ever this value is (eg `undefined`) in most cases, except:
   *
   * - `number` becomes `NaN`
   * - `BigInt` also becomes `NaN`
   * - `Symbol` becomes `_z.NOTHING` (i.e `Symbol('_z.NOTHING')`)
   * - `Date` becomes `new Date(null)`, which is 1970-01-01T00:00:00.000Z
   * - `Function` becomes a `() => EMPTY` (i.e `() => Symbol('_z.NOTHING')`)
   *
   ...
   *
   * @default: _z.NOTHING (i.e `Symbol('_z.NOTHING')`)
   */
  singlesReject?: any
}
...
export const LOOP_SYMBOL = Symbol('_z.loop')
...
>(
  ...
  if (isSingleOrWeak(input) && strict) {
    const errorMsg = `_z.loop(): strict = true, but wrong _z.isMany type. With type(value) === '${type(input)}' & value =`
  ...
  const addLoopGeneratorMethods = <TGen extends Generator | AsyncGenerator>(
    ...
    generator['each'] = () => {
      throw new Error(`ZenDash: loop().each() is not implemented yet!`)
    }
    generator['stop'] = () => {
      throw new Error(`ZenDash: loop().stop() is not implemented yet!`)
    }
    if (isAnyJustIterator(input) && _options.props !== true) {
      ...
      generator['restart'] = () => {
        throw new Error(`ZenDash: loop().restart() is not implemented yet!`)
      }
      generator['reloop'] = () => {
        throw new Error(`ZenDash: loop().reloop() is not implemented yet!`)
      }
    }
  ...
  const resultGenerator: LoopGenerator = (function* () {
    ...
    // Array and arguments are similar
      ...
      if (!_options.dataViewType)
        throw new Error(
          `_z.loop(): options.dataViewType is required to loop over an ArrayBuffer`
        )
    ...
    if (!handled) {
      const errorMsg = `_z.loop(): Internal Error: unsupported input value = `
