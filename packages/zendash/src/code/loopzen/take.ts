...
/**
 * Takes `n` elements from the beginning of the value (`Array`, `Object`, `Set`, `Map` etc). If `n` is a function, it stops as soon as it returns `false` or `_z.STOP`.
 *
 ...
 *
 * @see [`_z.filter`](/functions/filter.html) for filtering elements - the `filterSingles` option also applies here: if `take` is used with `isSingle` values, `filterSingles` must be `true` or it will throw an error.
 *
 ...
 *
 * @param n the number of elements to take. If a `function`, it will be called with `(item, keyIdx, value, count)`. If it returns `true` it will take items, as soon as it returns `false` or `_z.STOP` it stops taking items.
 *
...
export function takeToFunction<Titem, TidxKey, Tinput>(
  ...
  if (isNumber(take)) {
    ...
    const errorMsg = `_z.loop(): options.take must be a number or a function, but got type = "${type(take)}" value = ${take}`
