...
export interface IKeysOptions {
   ...
   *
   * By default, `_z.keys()` returns the "natural keys" for **special objects** with nested values:
   ...
   *
   * Throws if `dataViewType` is  missing and _z.type(input) === 'ArrayBuffer'
   */
...
export type KeysOrValues<
  ...
  IsBasicProps = false,
> =
  // Note: the `Exclude<T, never>` is a trick to force TS to report actual keys, instead of _z.SomeTypeHelper<...blah blah>. Doesn't work perfectly, but better than nothing!
 ...
 *
 *           _z.keys({ a: 1, b: 2, c: 3 })           //=> ['a', 'b', 'c'] & type is ('a' | 'b' | 'c')[]
 *
 *    - returns an `Array`'s indexes (by default)
 *
 *           _z.keys(['a', 'b', 'c'])                //=> [0, 1, 2 ] & type is number[]
 *
 *    - returns `Set` keys (by default)
 *
 *           _z.keys(new Set(['a', 'b', 'c']))       //=> ['a', 'b', 'c'] & type is ('a' | 'b' | 'c')[]
 *
 *    - returns `Map` keys (by default)
 *
 *           _z.keys(new Map([['a', 1], ['b', 2], ['c', 3]])) //=> ['a', 'b', 'c'] & type is ('a' | 'b' | 'c')[]
 *
 ...
 *      _z.keys(myArray, { props: true }) // -> ['stringProp']
 *
 *      // Optionally symbol keys as well
 *      _z.keys(myArray, { props: true, symbol: true}) // -> [ 'stringProp', Symbol(symbolProp) ]
 *
 ...
 *
 * # ZenDash `_z.keys()` rationale & stance
 *
 * # Typings Rationale
 *
 ...
 *
 * So, the rule is: just don't bother with **extra keys** you might receive. Either ignore them OR handle them gracefully, without throwing! Especially if you're dealing with an `_z.isInstance()` object (eg `Person`) (or a similar interface), you should NOT be able to access any other member, and that's how it works with all mainstream OO languages! When you're iterating over it for any reason (via `_z.keys()` or `_z.loop()`/`_z.each()`), you can always `switch` only on the keys you know, and ignore the rest.
 *
 * ## Historical Reasons
 *
 ...
 *
 * If you still insist on being pure on TS's dictation, then you can use the plain `_z.keysS()` function, which is exactly the same as `_z.keys()` (same implementation), but returns `string[]` keys (or `(string | symbol)[]` etc upon option's request), instead of the rigorously typed `_z.keys` (like `Object.keys()`).
 *
 * # Parameters
 *
...
export function keys(input: unknown, options: IKeysOptions = {}): any[] {
  ...
  if (!_.isObject(input)) {
    if (options.strict)
      throw new TypeError(
        `_z.keys: strict: true works only for types passing _.isObject, but value = ${toStringSafe(input)}`
      )

    if (isPrimitive(input)) return []
    else {
      const msg = `_z.keys: internal error: value is not isPrimitive, but not an _.isObject either! You might need to check logs for value.`
  ...
  if (isArrayBuffer(input)) {
    ...
    if (!options.props || options.props === 'all') {
      // prettier-ignore
      if (!options.dataViewType) throw new Error(`_z.keys(): options.dataViewType is required to loop over an ArrayBuffer to calculate its nested keys()`)
