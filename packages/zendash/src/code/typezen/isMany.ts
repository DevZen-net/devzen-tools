 ...
 *
 *
 * @note: On all of these "Many" types (except `WeakSet`/`WeakMap` which aren't revealing their items), you can use [`_z.loop(value)`](/functions/iterate.html) to get an `IterableIterator` of `[keyOrIdx, value]` pairs, which works the same way for all of them.
 *
 * It also has an option `allProps` that deals with props on all `_.isObject` values.
 *
 * @see [`_z.isSingle()`](/functions/isSingle.html) is the opposite of `isMany`
 *
 * @see [`_z.isPrimitive()`](/functions/isPrimitive.html) for the most basic single types
 *
 * @see [`_z.loop(value)`](/functions/iterate.html) to iterate on `isMany()` values (that makes sense).
 *
