 ...
 *
 * `NOTHING` is used to represent a real `never` value, used in the `_z.loop()` family functions to represent the absense of a value. We don't name it `never` (or `void`) so it's not inconsistent with TypeScript's typing system - but it might be renamed in the future.
 */
export const NOTHING = Symbol('_z.NOTHING')
export type T_NOTHING = typeof NOTHING

/**
 * Returns if value is a `_z.NOTHING`, i.e the absence of any value, in the ZenType/ZenDas world.
 *
 * @param value
 */
 ...
 * * and `value` is not equal to `_z.NOTHING`, `new Boolean(false)`, `new Number(0)`, `BigInt(0n)`, `new String('')`
