...
describe('_z.isSetEqual :', () => {
  describe('should compare Arrays of primitives & references by Reference', () => {
    ...
    _.each(testCases, ([a, b, expected]: [any[], any[], boolean]) =>
      it('using Default Comparator', () => expect(_z.isSetEqual(a, b)).equal(expected))
    )

    const refComparator = (a, b) => a === b
    _.each(testCases, ([a, b, expected]: [any[], any[], boolean]) =>
      it('using Custom Comparator', () =>
        expect(_z.isSetEqual(a, b, refComparator)).equal(expected))
    )
    ...
    _.each(
      ...
      ([a, b, expected, comparator]: [any[], any[], boolean, any]) => {
        it('should compare as Arrays', () =>
          expect(_z.isSetEqual(a, b, comparator)).equal(expected))

        it('should compare as Sets ', () =>
          expect(_z.isSetEqual(new Set(a), new Set(b), comparator)).equal(expected))
      }
    )
  })
})
