...
describe('isGenerator', () => {
  _.each(
    [
      ...
      ['Generator', iteratorsGenerators.Generator[0], true],

      // Async Generators - false
      ['a_GeneratorFunction_async_named', a_GeneratorFunction_async_named(), false],
      ['a_GeneratorFunction_async_anonymous', a_GeneratorFunction_async_anonymous(), false],
      ['AsyncGenerator', iteratorsGenerators.AsyncGenerator[0], false],

      // all Iterators false, including async
      ['AsyncIterator', iteratorsGenerators.AsyncIterator[0], false],
      ['Iterator', iteratorsGenerators.Iterator[0], false],
      ...
      ['normalFunction', a_Function_normalNamed(), false],
    ...
    ([descr, value, expected]: [string, any, boolean]) =>
      it(`${descr}: for ${String(value)} returns ${expected}`, () => {
        expect(isGenerator(value)).to.be.equal(expected)
      })
  )
})
