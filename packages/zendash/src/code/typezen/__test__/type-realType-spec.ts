...
describe('`_z.type` & its associates:', () => {
  describe('`_z.type` distinguishes all types', () => {
      ...
      describe(`${typeName}`, () => {
        for (const value of allTypesAndPossibleValues[typeName]) {
          it(`${toStringSafe(value)}`, () => {
            expect(_z.type(value)).to.equal(typeName)
          })
        }
      })
    }
  ...
  it('isSetEqual(TYPE_NAMES, typesOrder)', () => {
    expect(_z.isSetEqual(_z.TYPE_NAMES, TYPE_NAMES_ORDER)).to.be.true
  })
