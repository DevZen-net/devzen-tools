 ...
 *
 *      // primitives
 *      _z.isSetEqual([1, 2, 3], [3, 2, 1]) // true
 *      _z.isSetEqual([1, 2, 3], [4, 3, 2, 1]) // false
 *      _z.isSetEqual([1, 2, 3, 'John', 'Doe'], ['John', 3, 'Doe', 2, 1]) // true
 *
 *      // objects / refs
 *      _z.isSetEqual([{ a: 1 }, { b: 2 }, { c: 3 }], [{ c: 3 }, { b: 2 }, { a: 1 }]) // false
 *      _z.isSetEqual([{ a: 1 }, { b: 2 }, { c: 3 }], [{ c: 3 }, { b: 2 }, { a: 1 }], _.isEqual) // true
 *
 *      // custom comparator
 *      _z.isSetEqual(
 *        [{ a: 1 }, { b: 2 }, { c: 3 }],
...
export const isSetEqual = (
  ...
  if (set1Type !== set2Type)
    throw new Error(
      `_z.isSetEqual(): Both sets must be of the *same* type (Array or Set), but got '${set1Type}' & '${set2Type}'`
    )

  if (set1Type !== 'Set' && set1Type !== 'Array')
    throw new Error(
      `_z.isSetEqual(): Both sets must be of type (Array or Set), but got '${set1Type}' & '${set2Type}'`
    )
