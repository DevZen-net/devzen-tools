 ...
 *
 * @see `_z.isSingle` is similar to [`_z.isPrimitive()`](/functions/isPrimitive.html), but :
 *  - `_z.isSingle` also includes extras as "singles" (eg `Date` & `RegExp`), as they are not considered as having arbitrary props or contents (days months minutes etc in Date don't count as user-defined props).
 *  - `_z.isSingle` also includes boxed values of primitives (eg. `new Boolean(true)`), unlike `_z.isPrimitive()` which only allows for really primitive unboxed values.
 *
 * Think of it as lodash's *negated* `_.isObject()`, which unfortunately can't be used as such cause it fails in few cases:
 *
 *       _z.isSingle(new String()) === true
 *
 *   but
 *
 ...
 *
 * @see [`_z.isMany()`](/functions/isMany.html) is the opposite of `isSingle`
 *
 * @see [`_z.isPrimitive()`](/functions/isPrimitive.html) for the most basic single types
 *
